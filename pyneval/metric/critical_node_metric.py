...
def score_point_distance(
    ...
    gold_len, test_len, true_pos_num, false_neg_num, false_pos_num, mean_dis, tot_dis, pt_cost = get_result(
    ...
    get_colored_tree(test_node_list=test_node_list, gold_node_list=gold_node_list, switch=switch, km=km, color=color)
...
def critical_node_metric(gold_swc_tree, test_swc_tree, config):
    ...
    color = [config["true_positive"], config["missed"], config["excess"]]
    ...
    branch_result_tuple = score_point_distance(
